...
test <- read.csv("Datasets/ISarcasm/test.csv", fileEncoding = 'utf-8')
train <- read.csv("Datasets/ISarcasm/train.csv", fileEncoding = 'utf-8')
```
...
colnames(tSparse) = make.names(colnames(tSparse), unique = TRUE)
tSparse$class = train$class

tSparse <- tSparse[sample(1:nrow(tSparse)), ]
tSparse <- tSparse[1:1000,]
```
...
model <- train(
  class~., data = trainSet, method = "svmLinear",
