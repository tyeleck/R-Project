...
test <- read_csv(paste0(Dir,"/test.csv"))
y <- get_hashtags_df(test$tweets)
test$tweets <- test$tweets %>% sub(pattern = "#.* | #.*$", replacement = "")
test <- test %>% mutate(hashtags = y)
...
#load('E:/Github/R-Project/Datasets/test_w_hashtags.RData')
#test <- test %>% select(tweets,class)

write.csv(test, file = 'Datasets/Tweets_with_Sarcasm_and_Irony/test_without_hashtags.csv', fileEncoding = 'utf-8')
```
