...
retrieve_dataset_ISarcasm <- function(Dir = Dir_ISarcasm, binary = FALSE) {
  train <- read_csv(paste0(Dir,"/train.csv")) %>% rename(tweets = tweet, class = sarcastic)
  test <- read_csv(paste0(Dir,"/test.csv")) %>% rename(tweets = tweet, class = sarcastic)
...
<embed type="text/html" src="RNN_Models/LSTM_Tweets_with_Sarcasm/loss_acc.html" width="800" height="500">



```{r eval=FALSE}
save_model_hdf5(model, filepath = 'RNN_Models/LSTM_Tweets_with_Sarcasm/LSTM_Model.h5')
save_text_tokenizer(sequences$tokenizer, filename = 'RNN_Models/LSTM_Tweets_with_Sarcasm/tokenizer')
...
tokenizer <- load_text_tokenizer('RNN_Models/LSTM_Tweets_with_Sarcasm/tokenizer') 
